/* notepad_win32.c
   简易记事本：打开/保存/查找/替换，C89 兼容
   编译：gcc notepad_win32.c -o notepad_win32.exe -mwindows -lcomdlg32
   实现了基本的记事本功能，本人实测运行成功~
*/
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    hwnd = CreateWindowEx(WS_EX_CLIENTEDGE, "WindowClass", "简易记事本",
...
/* Simple modal input dialog: prompt single string. Returns 1 if OK and places text in outbuf */
int PromptString(HWND owner, const char *title, const char *label, char *outbuf, int outlen)
{
    ...
    dlg = CreateWindowEx(WS_EX_DLGMODALFRAME, "STATIC", title,
    ...
    while (GetMessage(&msg, NULL, 0, 0)) {
        ...
        if (msg.message == WM_COMMAND && LOWORD(msg.wParam) == 1) { /* OK */
            GetWindowText(edt, outbuf, outlen);
            result = 1;
            DestroyWindow(dlg);
            break;
        } else if (msg.message == WM_COMMAND && LOWORD(msg.wParam) == 2) { /* Cancel */
            result = 0;
            DestroyWindow(dlg);
            break;
        } else {
...
void DoFileOpen(HWND hwnd)
{
    ...
    if (!buf) { CloseHandle(hFile); MessageBox(hwnd, "内存不足", "错误", MB_OK); return; }

    if (!ReadFile(hFile, buf, fileSize, &readResult, NULL)) {
        free(buf); CloseHandle(hFile); MessageBox(hwnd, "读取文件失败", "错误", MB_OK); return;
    }
    buf[readResult] = '\0';
    CloseHandle(hFile);
...
void DoFileSave(HWND hwnd)
{
    ...
    if (!buf) { MessageBox(hwnd, "内存不足", "错误", MB_OK); return; }
    ...
    if (g_filename[0] == '\0') {
        ...
        if (!GetSaveFileName(&ofn)) { free(buf); return; }
        strncpy(g_filename, ofn.lpstrFile, MAX_PATH-1);
        g_filename[MAX_PATH-1] = '\0';
    }
...
void DoReplace(HWND hwnd)
{
    ...
    {
        int idx = (int)(pos - buf);
        int newlen = len - (int)strlen(findstr) + (int)strlen(repstr);
        char *newbuf = (char *)malloc((size_t)newlen + 1);
        if (!newbuf) { free(buf); MessageBox(hwnd, "内存不足", "错误", MB_OK); return; }
        memcpy(newbuf, buf, idx);
        memcpy(newbuf + idx, repstr, strlen(repstr));
        memcpy(newbuf + idx + strlen(repstr), buf + idx + strlen(findstr), len - idx - (int)strlen(findstr));
        newbuf[newlen] = '\0';
        SetWindowText(hedit, newbuf);
        SendMessage(hedit, EM_SETSEL, idx, idx + (int)strlen(repstr));
        SendMessage(hedit, EM_SCROLLCARET, 0, 0);
        free(newbuf);
    }
    free(buf);
